        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 shadow-sm">
          <div className="flex items-center justify-between border-b border-slate-800 pb-3 mb-4">
            <div>
              <h2 className="text-base font-bold text-slate-100 flex items-center gap-2">
                ...
                URTC Flasher Studio (CAN OTA)
              </h2>
              <p className="text-xs text-slate-400">
                Golden-Image A/B Backup Slot Architecture &bull; HMAC-SHA256 Signed OTA Updates
              </p>
            </div>
          </div>
          ...
          <div className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 text-xs text-slate-300 space-y-2 mb-5">
            <div className="flex items-center justify-between">
              <div className="font-bold text-amber-400 flex items-center gap-1.5">
                ...
                Connected Board (queried live via CAN 0x7F8/0x7F9/0x7FA)
              </div>
              <button
            ...
            {!isConnected && (
              <div className="text-[11px] text-slate-500">Connect the USB-CAN adapter (top header) to query the real board.</div>
            )}
            {isConnected && !boardVersion && (
              <div className="text-[11px] text-slate-500">No version reported yet - click "Query Version".</div>
            )}
            {boardVersion && (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[11px] font-mono">
                <div className="p-2 rounded bg-slate-900 border border-slate-800">
                  <div className="text-slate-500">Responder</div>
                  <div className="text-slate-200 font-semibold">{boardVersion.responder}</div>
                </div>
                <div className="p-2 rounded bg-slate-900 border border-slate-800">
                  <div className="text-slate-500">HardwareID</div>
                ...
                <div className="p-2 rounded bg-slate-900 border border-slate-800">
                  ...
                  <div className="text-slate-200 font-semibold">
                    {boardVersion.hardwareId === 0 ? 'none installed' : `${boardVersion.appMajor}.${boardVersion.appMinor}`}
                  </div>
                </div>
                <div className="p-2 rounded bg-slate-900 border border-slate-800">
                  <div className="text-slate-500">Bootloader Version</div>
                  <div className="text-slate-200 font-semibold">
                    {boardVersion.bootMajor !== undefined ? `${boardVersion.bootMajor}.${boardVersion.bootMinor}.${boardVersion.bootPatch}` : 'n/a (app running)'}
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="flex items-center justify-between pt-1 border-t border-slate-800/60">
              <span className="text-[11px] text-slate-400">CAN error counters (TEC/REC, 0x7FB/0x7FC)</span>
          ...
          <div className="mb-6 space-y-2">
            <div className="text-xs font-mono text-slate-400 flex items-center justify-between">
              <span>{flashTarget === 'main' ? 'STM32F303CC FLASH MEMORY MAP (256 KB)' : 'STM32F303CB FLASH MEMORY MAP (128 KB)'}</span>
            ...
            {flashTarget === 'main' ? (
              <div className="grid grid-cols-12 gap-1 text-[10px] font-mono text-center">
                <div className="col-span-2 p-2 rounded bg-amber-500/20 border border-amber-500/40 text-amber-300">
                  <div className="font-bold">BOOTLOADER</div><div>30 KB</div><div className="text-[9px] text-amber-400/80 mt-1">0x08000000</div>
                </div>
                <div className="col-span-1 p-2 rounded bg-slate-800 border border-slate-700 text-slate-300">
                  <div className="font-bold">META</div><div>2 KB</div><div className="text-[9px] text-slate-500 mt-1">0x08007800</div>
                </div>
                <div className="col-span-4 p-2 rounded bg-emerald-500/20 border border-emerald-500/40 text-emerald-300">
                  <div className="font-bold">MAIN SLOT (ACTIVE)</div><div>112 KB</div><div className="text-[9px] text-emerald-400/80 mt-1">0x08008000</div>
                </div>
                <div className="col-span-5 p-2 rounded bg-cyan-500/20 border border-cyan-500/40 text-cyan-300">
                  <div className="font-bold">BACKUP STAGING SLOT</div><div>112 KB</div><div className="text-[9px] text-cyan-400/80 mt-1">0x08024000</div>
                </div>
              </div>
            ) : (
              <div className="grid grid-cols-12 gap-1 text-[10px] font-mono text-center">
                <div className="col-span-3 p-2 rounded bg-amber-500/20 border border-amber-500/40 text-amber-300">
                  <div className="font-bold">BOOTLOADER</div><div>18 KB</div><div className="text-[9px] text-amber-400/80 mt-1">0x08000000</div>
                </div>
                <div className="col-span-4 p-2 rounded bg-emerald-500/20 border border-emerald-500/40 text-emerald-300">
                  <div className="font-bold">MAIN SLOT (ACTIVE)</div><div>54 KB</div><div className="text-[9px] text-emerald-400/80 mt-1">0x08005000</div>
                </div>
                <div className="col-span-5 p-2 rounded bg-cyan-500/20 border border-cyan-500/40 text-cyan-300">
                  <div className="font-bold">BACKUP STAGING SLOT</div><div>54 KB</div><div className="text-[9px] text-cyan-400/80 mt-1">~0x08013000</div>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="p-4 bg-slate-950 border border-slate-800 rounded-xl space-y-4">
            ...
            <div className="flex gap-2 border-b border-slate-800/60 pb-2 mb-3">
              <button onClick={() => setActiveFlasherTab('can-ota')} className={`px-3 py-1.5 text-xs font-semibold rounded border transition ${activeFlasherTab === 'can-ota' ? 'bg-slate-800 text-slate-200 border-slate-700' : 'bg-transparent text-slate-500 hover:text-slate-300 border-transparent hover:border-slate-800'}`}>CAN-OTA Programming</button>
              <button onClick={() => setActiveFlasherTab('swd')} className={`px-3 py-1.5 text-xs font-semibold rounded border transition ${activeFlasherTab === 'swd' ? 'bg-slate-800 text-slate-200 border-slate-700' : 'bg-transparent text-slate-500 hover:text-slate-300 border-transparent hover:border-slate-800'}`}>SWD/JTAG (desktop only)</button>
              <button onClick={() => setActiveFlasherTab('config')} className={`px-3 py-1.5 text-xs font-semibold rounded border transition ${activeFlasherTab === 'config' ? 'bg-slate-800 text-slate-200 border-slate-700' : 'bg-transparent text-slate-500 hover:text-slate-300 border-transparent hover:border-slate-800'}`}>Board Config</button>
            </div>
              ...
              <>
                <div className="flex gap-2">
                  <label className="flex-1 flex items-center gap-2 px-3 py-2 rounded-lg border cursor-pointer text-xs font-semibold transition"
                    ...
                    This board (main, direct CAN-OTA)
                  </label>
                  <label className="flex-1 flex items-center gap-2 px-3 py-2 rounded-lg border cursor-pointer text-xs font-semibold transition"
                    ...
                    Expansion slave (relayed via I2C, 0x210-0x219)
                  </label>
                </div>

                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1.5">Select Firmware Image (.bin):</label>
                  <div className="flex gap-2 items-center">
                    ...
                    >
                      <optgroup label="Local (public/firmware/)">
                      ...
                      {githubFirmwares.length > 0 && (
                        <optgroup label="GitHub Repository (JuanenRac/URTC)">
                      ...
                      {customFile && <option value={customFile.name}>{customFile.name} (browsed)</option>}
                    </select>
                    ...
                    <button onClick={() => fileInputRef.current?.click()} className="px-3 py-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-lg text-xs text-slate-300 transition whitespace-nowrap">Browse...</button>
                  </div>
                  {githubError && <div className="text-[10px] text-red-400 mt-1">GitHub listing: {githubError}</div>}
                </div>
                ...
                <div className="space-y-3 pt-2 pb-2">
                  <div className="flex flex-col gap-2 bg-slate-900 p-3 rounded-lg border border-slate-800">
                    <label className="flex items-start gap-2 text-xs text-slate-300 cursor-pointer">
                      ...
                      <div>
                        <span className="font-semibold block text-slate-200">Board is currently running the application (send enter-bootloader trigger first)</span>
                        <span className="text-[10px] text-slate-500 block leading-tight mt-0.5">Uncheck if the board is already sitting in its bootloader.</span>
                      </div>
                    </label>
                    ...
                    <label className={`flex items-start gap-2 text-xs cursor-pointer ${flashTarget === 'slave' ? 'opacity-40 pointer-events-none' : 'text-slate-300'}`}>
                      ...
                      <div>
                        ...
                        <span className="text-[10px] text-slate-500 block leading-tight mt-0.5">Main board only (no F-RAM on the expansion slave). Only takes effect while the trigger above also runs (F-RAM erase requires the application still running).</span>
                      </div>
                    </label>
                    ...
                    <label className={`flex items-start gap-2 text-xs cursor-pointer ${flashTarget === 'slave' ? 'opacity-40 pointer-events-none' : 'text-slate-300'}`}>
                      ...
                      <div>
                        <span className="font-semibold block text-slate-200">Allow downgrade (bypass anti-rollback, CAN 0x7FD)</span>
                        <span className="text-[10px] text-slate-500 block leading-tight mt-0.5">Main board only. One-shot per attempt - only lifts the version-ordering check, the full image still transfers and is verified normally.</span>
                      </div>
                    </label>
                  </div>
                </div>
                ...
                <div className="flex gap-2 pt-2">
                  <button
                    onClick={() => {
                      if (allowDowngrade && !window.confirm('You are about to authorize a firmware DOWNGRADE, bypassing the bootloader\'s anti-rollback protection. Continue?')) return;
                      if (!window.confirm(`Flash "${selectedFirmware}" to the ${flashTarget === 'slave' ? 'expansion slave' : 'main board'} now?`)) return;
                  ...
                  >
                    ...
                    <span>START CAN-OTA UPDATE ({flashTarget === 'slave' ? 'slave' : 'main'})</span>
                  </button>
                  {busy && (
                ...
                {flashTarget === 'main' && (
                  <button
                    ...
                    title="Reads the main slot's current contents back over CAN before you overwrite it (bootloader must already be running)"
                  >
                    <DownloadCloud className="w-4 h-4 text-cyan-400" />
                    <span>Backup Firmware (CAN readback, 0x7FE/0x7FF)</span>
                  </button>
                )}
                {!isConnected && (
                  <div className="text-[11px] text-amber-400/80 flex items-center gap-1.5">
                    <AlertOctagon className="w-3.5 h-3.5" /> Connect the USB-CAN adapter to enable flashing.
                  </div>
                )}
              </>
            )}
            ...
            {activeFlasherTab === 'swd' && (
              <div className="space-y-4">
                <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/30 text-amber-200 text-[11px] leading-relaxed">
                  A browser has no API to drive an SWD/JTAG probe (Web Serial only talks to serial-framed devices like a USB-CAN adapter, not a debug probe's own protocol). This tab exists to explain the exact commands the desktop <strong>URTC Flasher</strong> tool runs locally via STM32CubeProgrammer/pyOCD - it will not execute anything from here.
                </div>
                ...
                <div className="flex flex-col gap-2 pt-1 pb-1">
                  <label className="flex items-center gap-2 text-xs text-slate-300 cursor-pointer">
                    <input type="checkbox" checked={swdBackup} onChange={(e) => setSwdBackup(e.target.checked)} className="accent-amber-500" /> Backup existing flash contents before programming
                  </label>
                  <label className="flex items-center gap-2 text-xs text-slate-300 cursor-pointer">
                    <input type="checkbox" checked={swdDryRun} onChange={(e) => setSwdDryRun(e.target.checked)} className="accent-amber-500" /> Dry run (print the commands, execute nothing)
                  </label>
                </div>
                <button
                ...
                >
                  ...
                  <span>Show equivalent desktop command</span>
                </button>
              </div>
            )}
            ...
            {activeFlasherTab === 'config' && (
              <div className="space-y-4">
                <div className="bg-slate-900 border border-slate-800 p-4 rounded-lg">
                  <h3 className="text-sm font-semibold text-slate-200 mb-2">Expansion Board Type (0x1A0/0x1A1)</h3>
                  <div className="flex items-end gap-3">
                    <button onClick={() => queryAndSet(CAN_ID_EXPANSION_TYPE_RESP, 1500, (d) => setExpansionType(d[0]), 'expansion board type')} disabled={!isConnected} className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 disabled:opacity-40 border border-slate-700 rounded-lg text-xs text-slate-200 transition">Query</button>
                ...
                <div className="bg-slate-900 border border-slate-800 p-4 rounded-lg">
                  <h3 className="text-sm font-semibold text-slate-200 mb-2">MLX9064x Sensor Variant (0x1A6/0x1A7)</h3>
                  <div className="flex items-end gap-3">
                    <button onClick={() => queryAndSet(CAN_ID_MLX_VARIANT_RESP, 1500, (d) => setMlxVariant(d[0]), 'MLX sensor variant')} disabled={!isConnected} className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 disabled:opacity-40 border border-slate-700 rounded-lg text-xs text-slate-200 transition">Query</button>
                    <span className="text-xs text-slate-300 font-mono">{mlxVariant !== null ? MLX_SENSOR_VARIANTS[mlxVariant] ?? `unknown (${mlxVariant})` : '—'}</span>
                  </div>
                </div>

                <div className="bg-slate-900 border border-slate-800 p-4 rounded-lg">
                  <h3 className="text-sm font-semibold text-slate-200 mb-2">Free Tool Configuration (ID pins 11111)</h3>
                  <p className="text-[10px] text-slate-400 mb-3 leading-relaxed">Only takes effect on a board with all 5 ID jumpers installed. Saved to non-volatile memory.</p>
                  <div className="flex gap-2 mb-2">
                    ...
                    <button onClick={() => onSendFrame?.(CAN_ID_SET_FREE_TOOL, [parseInt(selectedFreeTool)], 'Program free tool config')} disabled={!isConnected} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 disabled:opacity-40 border border-slate-700 rounded-lg text-xs font-semibold text-slate-200 transition">Program</button>
                    <button onClick={() => queryAndSet(CAN_ID_FREE_TOOL_CONFIG_RESP, 1500, (d) => setFreeToolConfig(d[1] ?? null), 'free tool config')} disabled={!isConnected} className="px-3 py-2 bg-slate-800 hover:bg-slate-700 disabled:opacity-40 border border-slate-700 rounded-lg text-xs text-slate-200 transition">Query</button>
                  </div>
                  {freeToolConfig !== null && <div className="text-[11px] text-slate-400">Currently configured: {freeToolConfig === 0 || freeToolConfig > 25 ? 'none' : TOOL_PROFILES.find(t => t.id === freeToolConfig - 1)?.name ?? `#${freeToolConfig - 1}`}</div>}
                </div>

                <div className="bg-slate-900 border border-slate-800 p-4 rounded-lg">
                  <h3 className="text-sm font-semibold text-slate-200 mb-2">Peripheral Info & Serial Number (0x1A4/0x1A5)</h3>
                  <div className="flex items-end gap-3">
                    ...
                    <button onClick={() => queryAndSet(CAN_ID_PERIPHERAL_INFO_RESP, 1500, (d) => setPeripheralInfo({ type: d[0], serial: d[1] }), 'peripheral info')} disabled={!isConnected} className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 disabled:opacity-40 border border-slate-700 rounded-lg text-xs text-slate-200 transition">Query</button>
                    <button onClick={() => onSendFrame?.(0x1a4, [parseInt(serialNumber) & 0xFF], 'Program device serial')} disabled={!isConnected} className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 disabled:opacity-40 border border-slate-700 rounded-lg text-xs text-slate-200 transition">Program</button>
                  </div>
                  {peripheralInfo && <div className="text-[11px] text-slate-400 mt-2">Peripheral type: 0x{peripheralInfo.type.toString(16).padStart(2, '0')}{peripheralInfo.type === 0x03 ? ' (URTC)' : ''} &bull; Serial: {peripheralInfo.serial}</div>}
                </div>
              </div>
            )}
          ...
          <div className="flex-1 p-4 bg-slate-950 font-mono text-xs text-slate-300 space-y-1.5 overflow-y-auto max-h-[420px]">
            {flasherState.log.length === 0 ? (
              <div className="text-slate-500 italic">
                Ready. Select a firmware image and click "START CAN-OTA UPDATE".
              </div>
            ) : (
