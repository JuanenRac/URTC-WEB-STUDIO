          ...
          {isUnsupportedJumperInFW10 && (
            <div className="p-3.5 rounded-lg bg-red-500/10 border border-red-500/30 text-xs text-red-200 space-y-1 my-2">
              <div className="font-bold text-red-400">
                {t('hardware.unsupported_banner_title', '⚠️ UNSUPPORTED JUMPER ID #{{id}} IN FIRMWARE v0.0.0', { id: effectiveToolId })}
              </div>
              <p className="text-[11px] text-slate-300">
                {t('hardware.unsupported_banner_body', 'Jumpers select Tool ID #{{id}} ({{name}}). Firmware 0.0 only supports IDs 0 to 11. Flash Firmware v0.1.0 in Flasher Studio to use this tool profile.', { id: effectiveToolId, name: activeToolName })}
              </p>
            </div>
          )}
